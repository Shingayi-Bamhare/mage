...
(defmethod emit-data ::opcode
  [contexts {:keys [::opcode ::argument]}]
  (conj
    (clojure.core/pop contexts)
    (let [{:keys [::ilg ::type-builder ::labels ::locals ::fields] :as context} (last contexts)]
      (cond 
        (nil? argument)     (do (.Emit ilg opcode)
                              context)
        
        (seq? argument)  (let [ctx (reduce (fn [c token] (ensure-in-context c token))
                                              context argument)]
                              (do
                                (.Emit ilg opcode
...
(defmethod emit-data ::label
  ...
  (conj
    (clojure.core/pop contexts)
    (let [{:keys [::ilg ::labels] :as context} (last contexts)
          ^Label label (clojure.core/or (labels labelmap)
                                        (.DefineLabel ilg))]
      (.MarkLabel ilg label)
      (assoc-in context [::labels labelmap] label))))

(defmethod emit-data ::field
  [contexts {:keys [::field ::type ::attributes] :as fieldmap}]
  (conj
    (clojure.core/pop contexts)
    (let [{:keys [::type-builder] :as context} (last contexts)
          ^FieldBuilder field (.DefineField type-builder (str field) type attributes)]
...
(defmethod emit-data ::local
  ...
  (conj
    (clojure.core/pop contexts)
    (let [{:keys [::ilg] :as context} (last contexts)
          ^LocalBuilder local (.DeclareLocal ilg (::type localmap))]
...
(defmethod emit-data ::begin
  [contexts {:keys [::begin ::argument]}]
  (conj
    contexts
    (let [{:keys [::ilg ::assembly-builder ::module-builder ::type-builder] :as context} (last contexts)]
      (case begin
        ;; builders
...
(defmethod emit-data ::end
  [contexts {:keys [::end]}]
  (if-let [{:keys [::ilg ::assembly-builder ::type-builder] :as context} (last contexts)]
    (case end
      ;; builders
      ;; TODO always save?
      :assembly    (do
